...
import io.pepstore.swagger.tasks.GetUserTask;
...
public class GetUserStepDefinition {
    ...
    @When("The user consumes the endpoint and obtains the information")
    public void theUserConsumesTheEndpointAndObtainsTheInformation() {
        user.attemptsTo(
                GetUserTask.get()
        );
    }
    @Then("The user then validates the service response")
    public void theUserThenValidatesTheServiceResponse() {
        user.should(
                seeThat(
